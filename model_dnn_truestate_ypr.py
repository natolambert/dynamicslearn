...
class NeuralNet_ypr(nn.Module):
    ...
    def train(self, dataset, learning_rate = 1e-3, epochs=50, batch_size=50, optim="Adam", loss_fn=nn.MSELoss(), split=0.8, preprocess=True):
        """
        ...
        if(optim=="Adam"):
            optimizer = torch.optim.Adam(super(NeuralNet_ypr, self).parameters(), lr=learning_rate)
        elif(optim=="SGD"):
            optimizer = torch.optim.SGD(super(NeuralNet_ypr, self).parameters(), lr=learning_rate)
        else:
