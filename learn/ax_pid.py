...
def pid(cfg):
    ...
    def bo_rollout_wrapper(params, weights=None):  # env, controller, exp_cfg):
        ...
        pid_1 = [params["roll-p"], params["roll-i"], params["roll-d"]]
        pid_2 = [params["pitch-p"], params["pitch-i"], params["pitch-d"]]
        ...
        while r < cfg.experiment.repeat:
            ...
            #     continue
            rollout_cost = np.sum(rews) / cfg.experiment.repeat  # / len(rews)
        ...
        return cum_cost.reshape(1, 1), 0 #std
    ...
    exp = SimpleExperiment(
        ...
        search_space=SearchSpace([
            RangeParameter(
                name=f"roll-p", parameter_type=ParameterType.FLOAT, lower=10.0, upper=500.0
            ),
            RangeParameter(
                name=f"roll-d", parameter_type=ParameterType.FLOAT, lower=0.0, upper=500.0
            ),
            FixedParameter(name="roll-i", value=0.0, parameter_type=ParameterType.FLOAT),
            # RangeParameter(
            #     name=f"roll-i", parameter_type=ParameterType.FLOAT, lower=0.0, upper=500.0
            # ),
            RangeParameter(
                name=f"pitch-p", parameter_type=ParameterType.FLOAT, lower=10.0, upper=500.0
            ),
            RangeParameter(
                name=f"pitch-d", parameter_type=ParameterType.FLOAT, lower=0.0, upper=500.0
            ),
            # RangeParameter(
