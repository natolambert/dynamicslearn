...
class QuadEnv(gym.Env):
    ...
    def sample_random_state(self):
        ...
        while not acceptable:
            ...
            random_state_a = self.dyn_data.iloc[row_idx,
                             12 + 9 * self.num_stack + 4:12 + 9 * self.num_stack + 4 + 4 * (self.num_stack - 1)].values
    ...
    def next_state(self, state, action):
        ...
        new_action = np.concatenate(
            (action, state[9 * self.num_stack: 9 * self.num_stack + 4 * (self.num_stack - 2)]))  #
